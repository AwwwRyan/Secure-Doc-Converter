...
| Unlock (decrypt with known password; strip owner restrictions) | **`@neslinesli93/qpdf-wasm`** 0.3.0 (qpdf → WASM) | Apache-2.0 | `qpdf --decrypt [--password=…]`. RC4 + AES-128/256. `.wasm` vendored to `public/vendor/qpdf/` (`pnpm vendor`, `SHA256SUMS` in CI); JS glue imported via the bundler (`fs`/`path`/`crypto` are node-only dead branches that Vite externalises). Runs in `unlock.worker`. Refuses to guess/crack — wrong or missing password → clear error, no output. |
